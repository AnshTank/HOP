...
import { X, Send, Mic, MicOff, Volume2, VolumeX, Bot, User, Activity, Pill, Heart, AlertTriangle } from "lucide-react"
import type { ChatMessage, PatientDetails } from "@/frontend/types/patient"
...
export default function PatientDetailsAIModal({ isOpen, onClose, patient }: PatientDetailsAIModalProps) {
      ...
      <div className="bg-white w-full max-w-5xl h-[85vh] rounded-2xl shadow-2xl flex flex-col m-4">
        ...
        <div className="flex justify-between items-center p-6 border-b border-gray-200">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h2 className="text-xl font-bold text-purple-600">AI Clinical Assistant</h2>
              <p className="text-sm text-gray-600">
                Comprehensive care guidance for {patient.name} - Room {patient.room}
              </p>
            </div>
          </div>
        ...
        <div className="p-4 border-b border-gray-100 bg-gray-50">
          <div className="flex flex-wrap gap-2">
            <Button
              variant="outline"
              size="sm"
              onClick={() => handleQuickAction("What are the current vital signs concerns?")}
            ...
            <Button
              variant="outline"
              size="sm"
              onClick={() => handleQuickAction("Review medication schedule and interactions")}
              className="rounded-xl text-xs"
            >
            ...
            <Button
              variant="outline"
              size="sm"
              onClick={() => handleQuickAction("What are the priority nursing interventions?")}
              className="rounded-xl text-xs"
            >
            ...
            <Button
              variant="outline"
              size="sm"
              onClick={() => handleQuickAction("Assess discharge readiness and planning")}
              className="rounded-xl text-xs"
            >
            ...
            {patient.allergies.length > 0 && (
              <Button
                variant="outline"
                size="sm"
                onClick={() => handleQuickAction("Review allergy precautions and safety measures")}
...
function generateWelcomeMessage(patient: PatientDetails): string {
  ...
  if (patient.riskLevel === "critical") {
    criticalAlerts.push("🚨 CRITICAL risk level - requires continuous monitoring")
  }

  if (patient.acuityLevel >= 4) {
    criticalAlerts.push(`⚠️ High acuity level ${patient.acuityLevel} - frequent assessments needed`)
  }
  ...
  if (patient.vitals.painLevel && patient.vitals.painLevel > 5) {
    vitalsConcerns.push(`Pain level ${patient.vitals.painLevel}/10 - requires intervention`)
  }

  if (patient.vitals.temperature && (patient.vitals.temperature > 101 || patient.vitals.temperature < 96)) {
    vitalsConcerns.push(`Temperature ${patient.vitals.temperature}°F - monitor closely`)
  }

  const medicationAlerts = []
  const activeMeds = patient.medications.filter((med) => med.status === "active")
  if (activeMeds.length > 0) {
    medicationAlerts.push(`${activeMeds.length} active medications - check for interactions`)
  }
...
🏥 Room: ${patient.room} | Age: ${patient.demographics.age} | ${patient.demographics.gender}
...
${vitalsConcerns.length > 0 ? `**VITAL SIGNS CONCERNS:**\n${vitalsConcerns.map((concern) => `• ${concern}`).join("\n")}\n\n` : ""}

${medicationAlerts.length > 0 ? `**MEDICATION STATUS:**\n${medicationAlerts.map((alert) => `• ${alert}`).join("\n")}\n\n` : ""}
...
function generateDetailedAIResponse(message: string, patient: PatientDetails): string {
  const lowerMessage = message.toLowerCase()

  // Vital signs analysis
  if (lowerMessage.includes("vital") || lowerMessage.includes("assessment") || lowerMessage.includes("monitor")) {
...
🔹 **Frequency**: Acuity ${patient.acuityLevel} requires vitals every ${patient.acuityLevel >= 4 ? "1-2 hours" : patient.acuityLevel === 3 ? "4 hours" : "6-8 hours"}
🔹 **Priority Parameters**: Focus on trends, not just individual readings
🔹 **Alert Thresholds**: Given ${patient.primaryDiagnosis}, watch for specific warning signs
...
• Consider pain management if pain score >5`
  ...
  // Discharge planning
  if (lowerMessage.includes("discharge") || lowerMessage.includes("home") || lowerMessage.includes("ready")) {
    return generateDischargeGuidance(patient)
  }

  // Allergy safety
  if (lowerMessage.includes("allerg") || lowerMessage.includes("safety") || lowerMessage.includes("precaution")) {
    return generateAllergyGuidance(patient)
  }

  // Assessment guidance
  if (lowerMessage.includes("assess") || lowerMessage.includes("exam") || lowerMessage.includes("check")) {
...
⚡ **Risk Level**: ${patient.riskLevel.toUpperCase()} (requires ${patient.riskLevel === "critical" ? "continuous monitoring" : "regular assessment"})
📊 **Acuity**: Level ${patient.acuityLevel} (${patient.acuityLevel >= 4 ? "high priority" : "standard monitoring"})
...
${patient.allergies.length > 0 ? `• ALLERGY ALERT: ${patient.allergies.join(", ")}` : ""}
...
Would you like specific guidance on any particular aspect of ${patient.name}'s care?`
}
...
function analyzeVitals(patient: PatientDetails): string {
  ...
  if (vitals.temperature) {
    if (vitals.temperature > 101) {
      concerns.push(`🌡️ **FEVER**: ${vitals.temperature}°F - Consider antipyretics, cooling measures, infection workup`)
    } else if (vitals.temperature < 96) {
      concerns.push(`🧊 **HYPOTHERMIA**: ${vitals.temperature}°F - Warming measures needed, assess circulation`)
    } else {
  ...
  if (vitals.bloodPressure) {
    ...
    if (systolic > 140 || diastolic > 90) {
      concerns.push(`📈 **HYPERTENSION**: ${systolic}/${diastolic} mmHg - Monitor closely, consider antihypertensives`)
    } else if (systolic < 90 || diastolic < 60) {
      concerns.push(`📉 **HYPOTENSION**: ${systolic}/${diastolic} mmHg - Assess fluid status, consider IV fluids`)
    } else {
  ...
  if (vitals.heartRate) {
    if (vitals.heartRate > 100) {
      ...
      concerns.push(`💓 **BRADYCARDIA**: ${vitals.heartRate} bpm - Monitor for symptoms, assess medications`)
    } else {
  ...
  if (vitals.painLevel && vitals.painLevel > 0) {
    if (vitals.painLevel >= 7) {
      concerns.push(`😣 **SEVERE PAIN**: ${vitals.painLevel}/10 - Immediate intervention needed`)
    } else if (vitals.painLevel >= 4) {
      concerns.push(`😐 **MODERATE PAIN**: ${vitals.painLevel}/10 - Pain management indicated`)
    } else {
...
function generateMedicationGuidance(patient: PatientDetails): string {
  ...
  if (activeMeds.length > 0) {
    ...
    activeMeds.forEach((med) => {
      ...
      guidance += `   • Last given: ${med.lastGiven ? new Date(med.lastGiven).toLocaleString() : "Never"}\n\n`
    })
  }
  ...
  if (patient.allergies.length > 0) {
    guidance += `🚨 **ALLERGIES**: ${patient.allergies.join(", ")} - Verify before any new medications\n`
  }
...
**CURRENT PAIN STATUS:**
😣 **Pain Level**: ${currentPain}/10 ${currentPain >= 7 ? "(SEVERE - Immediate intervention needed)" : currentPain >= 4 ? "(MODERATE - Management indicated)" : currentPain > 0 ? "(MILD - Monitor closely)" : "(No pain reported)"}
...
${patient.allergies.length > 0 ? `• Allergy precautions: ${patient.allergies.join(", ")}` : ""}
...
${patient.allergies.length > 0 ? `• Allergies: ${patient.allergies.join(", ")} - ensure awareness` : ""}`
}
...
🚨 **CRITICAL ALLERGIES:**
${patient.allergies.map((allergy) => `• **${allergy.toUpperCase()}** - Verify before ANY intervention`).join("\n")}
...
• **Acuity ${patient.acuityLevel}**: Every ${patient.acuityLevel >= 4 ? "1-2 hours" : patient.acuityLevel === 3 ? "4 hours" : "6-8 hours"}
• **Risk Level ${patient.riskLevel.toUpperCase()}**: ${patient.riskLevel === "critical" ? "Continuous monitoring" : "Regular scheduled assessments"}
