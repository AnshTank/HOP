...
import { Heart, Clock, Pill, Shield, Calendar, Mic, Volume2, Activity, AlertTriangle, Bed, Bot } from "lucide-react"
import type { PatientBasic, PatientStatus } from "@/frontend/types/patient"
...
export function WarmPatientCard({ patient, status, isSelected, onSelect, onStartHandoff }: WarmPatientCardProps) {
  ...
  const handleStartHandoff = (e: React.MouseEvent) => {
    e.stopPropagation()
    // Show AI Assistant for handoff preparation
    setShowAIAssistant(true)
  }

  const handleNavigateToDetails = () => {
    // Close AI assistant and navigate to patient details
  ...
  return (
    <>
      ...
      >
        ...
        <CardContent className="p-6 flex flex-col h-full">
          ...
          <div className="flex items-center gap-2 text-sm mb-4">
            <Badge
              variant="outline"
              className="bg-white/80 border-blue-200 text-blue-700 rounded-xl px-3 flex-shrink-0"
            >
              Room {patient.room}
            </Badge>
          ...
          <div className="grid grid-cols-2 gap-3 mb-4 flex-grow">
            ...
            <div className="bg-white/60 rounded-2xl p-3 border border-gray-100">
              ...
              <div className="text-sm font-semibold text-gray-800">
                {patient.lastVitalsTime ? getTimeAgo(patient.lastVitalsTime) : "Not recorded"}
              </div>
            </div>
          </div>
      ...
      {/* AI Assistant Modal for Handoff */}
      <AIAssistantModal
